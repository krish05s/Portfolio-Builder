...
export default function MainDetails(props) {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get(`https://portfolio-builder-0axj.onrender.com/api/main/1/`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isUpdating) {
        await axios.put(`https://portfolio-builder-0axj.onrender.com/api/main/${userId}/`, data, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
        props.showAlert("Data Updated Successfully", "success");
      } else {
        await axios.post(`https://portfolio-builder-0axj.onrender.com/api/main/`, data, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
